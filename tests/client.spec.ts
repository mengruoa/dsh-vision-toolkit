...
afterEach(() => {
  cleanup()
  document.querySelectorAll('style[data-plugin-css="@mengruo/dsh-vision-toolkit/client"]').forEach(element => { element.remove() })
  ...
  return {
    ...
    settings: {
      value: {
        ...
        sessionMaxConcurrency: 6,
        minAvailableSeconds: 20,
...
describe('Vision Toolkit client plugin', () => {
  ...
  it('uses Harness theme tokens for every theme-dependent color', () => {
    ...
    const styles = document.querySelector<HTMLStyleElement>('style[data-plugin-css="@mengruo/dsh-vision-toolkit/client"]')
