...
import { resolveWorkspaceStorage } from "./paths.js";
 ...
 */
export async function sessionPasteRoot(ctx, sessionId, storageDir) {
...
async function copyImage(source, directory, filename) {
    const id = randomUUID();
    const finalPath = join(directory, `${id}-${filename}`);
    const stagingPath = join(directory, `.${id}.partial`);
    ensurePathInside(directory, finalPath);
