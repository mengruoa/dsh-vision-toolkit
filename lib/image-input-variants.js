...
import { createEvidenceCacheKey, EvidenceCache, SessionEvidenceStore, } from "./evidence-cache.js";
import { sessionPasteRoot } from "./paste-images.js";
export { EvidenceCache } from "./evidence-cache.js";
...
function retryPolicyKey(policy) {
    ...
    if (policy.mode === 'always')
        return JSON.stringify([policy.mode, ...backoff]);
...
async function materializeImage(ctx, block, data, extension, sessionId, storageDir) {
 ...
 */
async function readImageBlock(ctx, runtime, block, query, sessionId, storageDir) {
 ...
 */
export async function convertImagesToEvidence(ctx, runtime, cache, messages, signal, sessionId, runtimeHash = 'process-only-runtime', storageDir) {
    const session = sessionId === undefined ? undefined : ctx.sessions.get(sessionId);
    ...
    const converted = await Promise.all(plans.map(async ({ message, query }) => {
        if (query === undefined)
            return message;
        const content = await convertBlocks(message.content, (block) => abortableWait(limit(() => cache.read(createEvidenceCacheKey({
            ...(sessionId === undefined ? {} : { sessionId }),
            ...(sessionIdentity === undefined ? {} : { sessionIdentity }),
            attachmentId: String(block.attachment.attachmentId),
            prompt: query,
            runtimeHash,
        }), () => readImageBlock(ctx, runtime, block, query, sessionId, storageDir)), signal), signal));
        return { ...message, content };
    }));
...
export class ImageInputVariantAdapter extends LlmAdapter {
    ...
    cache;
    hidden;
    startupStorageDirectory;
    constructor(ctx, llm, upstream, upstreamName, runtime, cache, hidden = () => false, startupStorageDirectory = () => undefined) {
        super();
        this.ctx = ctx;
        this.llm = llm;
        ...
        this.cache = cache;
        this.hidden = hidden;
        this.startupStorageDirectory = startupStorageDirectory;
    }
    ...
    async *stream(options) {
        ...
        const messages = await convertImagesToEvidence(this.ctx, () => captured, this.cache, options.messages, options.signal, options.sessionId === undefined ? undefined : String(options.sessionId), captured?.evidenceFingerprint ?? createHash('sha256')
            .update('process-only-runtime')
            .update('\0')
            .update(storageDir ?? '')
            .digest('hex'), storageDir);
 ...
 */
export function installImageInputVariants(ctx, getConfig, getRuntime, getStartupStorageDirectory = () => undefined) {
            ...
            for (const provider of providers) {
                ...
                if (registration !== undefined && live.has(variantId)) {
                    ...
                    if (liveNow.has(variantId)) {
                        if (eligible && registration.retryPolicyKey === upstreamRetryPolicyKey)
                            continue;
                ...
                if (disposed)
                    return;
                try {
                    const dispose = llm.registerAdapter([variantId], new ImageInputVariantAdapter(ctx, llm, upstream, provider.name, getRuntime, evidenceCache, () => getConfig().imageInputVariants.hidden, getStartupStorageDirectory));
