 ...
 */
export async function withWindowsTransientRetry(operation) {
    let lastError;
 ...
 */
export async function ignoreCleanupFailure(ctx, label, path) {
    try {
        await withWindowsTransientRetry(() => rm(path, { recursive: true, force: true }));
    }
    catch (error) {
        ctx.logger.warn('dsh-vision-toolkit: %s cleanup failed: %s', label, error instanceof Error ? error.message : String(error));
    }
}
function runtimeGarbageLockToken(lockBase) {
    return sha256(lockBase).slice(0, 12);
}
function managedRuntimeGarbage(name) {
...
async function garbageCollectDirectory(ctx, parent, classify, now) {
    let entries;
    try {
        ...
        if (error.code === 'ENOENT')
            return;
        ctx.logger.warn('dsh-vision-toolkit: runtime garbage collection scan failed for %s: %s', parent, error instanceof Error ? error.message : String(error));
        return;
    }
    let activeLockTokens;
    const getActiveLockTokens = async () => {
        if (activeLockTokens !== undefined)
            return activeLockTokens;
        activeLockTokens = new Set();
    ...
    for (const entry of entries) {
        ...
        if (candidate.lockName !== undefined) {
            if (await runtimeLockIsActive(join(parent, candidate.lockName), now))
                continue;
        }
        else if (candidate.lockToken !== undefined) {
            if ((await getActiveLockTokens()).has(candidate.lockToken))
                continue;
        }
        else {
            ...
            if (!legacyCollectionSafe)
                continue;
            try {
                const info = await stat(path);
                if (now - info.mtimeMs <= LEGACY_RUNTIME_GC_STALE_MS)
                    continue;
            }
            catch (error) {
                if (error.code === 'ENOENT')
                    continue;
                ctx.logger.warn('dsh-vision-toolkit: runtime garbage collection inspection failed for %s: %s', path, error instanceof Error ? error.message : String(error));
                continue;
            }
        }
        if (candidate.minimumAgeMs !== undefined) {
            ...
            if (now - createdAt <= candidate.minimumAgeMs)
                continue;
        }
 ...
 */
export async function garbageCollectRuntimeCache(ctx, stateRoot, now = Date.now()) {
...
async function installDependenciesWithFallback(ctx, argv, stateRoot, env, label) {
    let lastResult;
    ...
    throw new VisionToolkitError('runtime', `${label} failed to install managed runtime dependencies: ${(lastResult?.stderr ?? '').trim()}`);
}
    ...
    try {
        for (let redirects = 0;; redirects++) {
            ...
            if (!DOWNLOAD_HOSTS.has(host))
                throw new Error(`download redirected outside the allowlist: ${host}`);
...
async function downloadBundledPythonOnce(url, artifact, destination, requestImpl = defaultDownloadRequest) {
...
async function downloadBundledPython(manifest, artifact, destination, requestImpl) {
    const sources = [
...
async function prepareManaged(ctx, config, manifest) {
    ...
    try {
        ...
        if (bootstrap.command.prefix.length === 0) {
            try {
                ...
                if (uv.exitCode === 0 && !uv.timedOut) {
                    ...
                    await installDependenciesWithFallback(ctx, ['uv', 'pip', 'install', '--python', venvPython(staging), '--requirement', REQUIREMENTS_PATH], stateRoot, installEnv, 'uv');
                    manager = 'uv';
                    created = true;
                }
            }
        ...
        if (!created) {
            ...
            await installDependenciesWithFallback(ctx, [venvPython(staging), '-m', 'pip', 'install', '--disable-pip-version-check', '--no-input', '-r', REQUIREMENTS_PATH], stateRoot, installEnv, 'pip');
        }
        ...
        await releaseManagedLock(lockPath, lockOwner).catch(error => {
            ctx.logger.warn('dsh-vision-toolkit: managed runtime lock cleanup failed: %s', error instanceof Error ? error.message : String(error));
        });
    }
}
