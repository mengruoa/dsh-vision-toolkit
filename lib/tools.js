...
function visibleBySnapshot(definition, snapshot) {
...
function callOptions(exec, timeoutSeconds, lifecycleSignal) {
...
const requiredNullableStringSchema = { ...nullableStringSchema, required: true };
const requiredNullableIntegerSchema = { ...nullableIntegerSchema, required: true };
const requiredNullableNumberSchema = { ...nullableNumberSchema, required: true };
 ...
 */
export function createVisionTools(source, projectPresentation = presentationIdentity, lifecycleSignal, toolVisibility = { local: true, online: true, video: false }) {
    ...
    const tools = [
        ...
        defineTool({
            ...
            async execute(args, exec) {
                const request = { video: args.video };
