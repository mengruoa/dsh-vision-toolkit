...
import { resolveConfig, retainedStorageHistory, isBuiltInFreeVisionProvider, VISION_TOOLKIT_SETTINGS_NAMESPACE, } from "./config.js";
...
function parseRequest(value) {
    ...
    if (value.action === 'health') {
        ...
        if (providerIndex !== undefined && (!Number.isSafeInteger(providerIndex) || providerIndex < 0)) {
        ...
        return {
            ...
            testModel,
            ...(providerIndex === undefined ? {} : { providerIndex: providerIndex }),
        };
    }
    ...
    if (value.action === 'test-storage')
        return { action: 'test-storage' };
...
export class VisionToolkitWebBackend {
    ...
    async save(request) {
        ...
        const current = resolveConfig(descriptorOf(this.ctx).value);
    ...
    async saveCredential(request) {
        ...
        if (provider === undefined) {
            throw new CredentialReferenceConflictError(`credential reference "${ref}" does not match any configured vision provider; reload Settings and try again`);
        }
        if (isBuiltInFreeVisionProvider(provider)) {
    ...
    async health(request, req) {
        ...
        try {
            ...
            if (request.providerIndex !== undefined) {
                const resolved = resolveConfig(descriptorOf(this.ctx).value);
